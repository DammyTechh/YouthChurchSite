...
import {
  ...
  Lock, Mail, User, Key, FileText, Search,
...
const AdminPanel = () => {
  ...
  useEffect(() => {
    if (user) { fetchSocialPosts(); fetchEvents(); fetchBlogPosts(); fetchMembers(); }
  }, [user]);
    ...
    <div className="flex flex-col h-full bg-[#111120] border-r border-[#C9A84C]/12">
      <div className="p-5 border-b border-white/6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg overflow-hidden ring-1 ring-[#C9A84C]/30">
  ...
  const PostsTab = () => {
    ...
    useEffect(() => {
      if (editPost) setForm({ title: editPost.title, description: editPost.description, platform: editPost.platform, url: editPost.url, thumbnail: editPost.thumbnail || '', date: editPost.date });
    ...
    const save = async () => {
      ...
      const autoThumb = form.thumbnail.trim() || getAutoThumbnail(form.url, form.platform);
      const payload = { ...form, thumbnail: autoThumb };
      const { error } = isEditing
        ? await supabase.from('social_media_posts').update(payload).eq('id', editPost!.id)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Title *</label>
                <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} className="input-field" placeholder="Post title" /></div>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">URL *</label>
                <input value={form.url} onChange={e => setForm(p => ({ ...p, url: e.target.value }))} className="input-field" placeholder="https://..." />
                <p className="text-xs text-[#A8A6A0] mt-1">YouTube links will auto-generate a thumbnail preview</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">
                  Thumbnail URL <span className="normal-case text-[#C9A84C]/70">(auto for YouTube)</span>
                </label>
                ...
                {previewThumb && (
                  <div className="mt-2 relative rounded-lg overflow-hidden border border-[#EEEAE4] h-32">
                    <img src={previewThumb} alt="Preview" className="w-full h-full object-cover" onError={e => (e.currentTarget.style.display = 'none')} />
                    <div className="absolute top-2 left-2 text-xs bg-black/60 text-white px-2 py-0.5 rounded">Preview</div>
                  </div>
                )}
              </div>
              <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Description</label>
                <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className="input-field resize-none" rows={3} placeholder="Post description" /></div>
              <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Date</label>
                <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className="input-field" /></div>
            </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {socialPosts.map(post => {
            ...
            const platColors: Record<string, string> = { youtube: '#dc2626', instagram: '#9333ea', tiktok: '#111', facebook: '#2563eb' };
            return (
              <div key={post.id} className="bg-white rounded-xl border border-[#EEEAE4] overflow-hidden">
                <div className="relative w-full h-40 bg-[#F8F7F4]">
                  {autoThumb
                    ? <img src={autoThumb} alt={post.title} className="w-full h-40 object-cover" onError={e => { e.currentTarget.style.display = 'none'; }} />
                    : <div className="w-full h-40 flex items-center justify-center" style={{ background: platColors[post.platform] || '#666' }}>
                      {platformIcon(post.platform) && <span className="text-white opacity-40 scale-[4]">{platformIcon(post.platform)}</span>}
                    </div>
                  }
                  <a href={post.url} target="_blank" rel="noopener noreferrer"
                    className="absolute inset-0 flex items-center justify-center bg-black/0 hover:bg-black/20 transition-all group">
                    <span className="opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 text-[#1A1A2E] text-xs font-semibold px-3 py-1.5 rounded-full">Open Link ↗</span>
                  </a>
                </div>
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-white text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1"
                      style={{ background: platColors[post.platform] || '#666' }}>
  ...
  const EventsTab = () => {
    ...
    const save = async () => {
      ...
      const { error } = isEditing
        ? await supabase.from('upcoming_events').update(form).eq('id', editEvent!.id)
        : await supabase.from('upcoming_events').insert([form]);
      if (error) { notify('error', error.message); return; }
    ...
    const Form = () => (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
        <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div className="p-6 space-y-4">
            <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Title</label>
              <input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} className="input-field" placeholder="Event title" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Date</label>
                <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} className="input-field" /></div>
              <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Time</label>
                <input type="time" value={form.time} onChange={e => setForm(p => ({ ...p, time: e.target.value }))} className="input-field" /></div>
            </div>
            <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Location</label>
              <input value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))} className="input-field" placeholder="Venue / location" /></div>
            <div><label className="block text-xs font-semibold text-[#A8A6A0] uppercase tracking-wider mb-1.5">Description</label>
              <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className="input-field resize-none" rows={3} placeholder="Event details" /></div>
            <div>
  ...
  const BlogTab = () => {
    ...
    const MEDIA_TYPES: { value: UiMediaType; label: string }[] = [
    ...
    useEffect(() => {
      if (editBlog) setForm({
        ...
        media_url: editBlog.media_url || '',
        // Map DB type back to UI type — DB stores 'image' for both image and flyer
        media_type: (editBlog.media_type as UiMediaType) || 'text',
        author_name: editBlog.author_name || 'RuggedYouth Team',
        tags: editBlog.tags || [],
        is_published: editBlog.is_published,
        like_count: editBlog.like_count || 0
      });
    }, []); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const uploadFile = async (file: File) => {
      ...
      const autoType: UiMediaType = file.type.startsWith('image') ? (form.media_type === 'flyer' ? 'flyer' : 'image')
        : file.type.startsWith('video') ? 'video'
          : file.type.startsWith('audio') ? 'audio'
            : 'text';
      setForm(p => ({ ...p, media_url: publicUrl, media_type: autoType }));
      setUploading(false);
      setUploadProgress('');
    ...
    // Map UI media type → DB media type before saving
    const toDbMediaType = (t: UiMediaType): DbMediaType => {
    ...
    const save = async () => {
      ...
      const payload = {
        ...form,
        media_type: toDbMediaType(form.media_type),
    ...
    const loadComments = async (postId: string) => {
      ...
      setPostComments((data as BlogComment[]) || []);
      ...
      <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-6 bg-black/70 overflow-y-auto">
        <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl my-4">
          ...
          <div className="p-7 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {MEDIA_TYPES.map(mt => (
                  <button key={mt.value} type="button"
                    onClick={() => setForm(p => ({ ...p, media_type: mt.value, media_url: mt.value === 'text' ? '' : p.media_url }))}
            ...
            {/* Media upload for image, video, audio, flyer */}
            {(['image', 'video', 'audio', 'flyer'] as UiMediaType[]).includes(form.media_type) && (
              <div>
                ...
                <input type="file"
                  accept={form.media_type === 'video' ? 'video/*' : form.media_type === 'audio' ? 'audio/*' : 'image/*'}
            ...
            <div>
              <label className="block text-xs font-bold text-[#A8A6A0] uppercase tracking-wider mb-1.5">
                Preview Excerpt <span className="normal-case text-[#C9A84C]/70">(auto if blank)</span>
              </label>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button onClick={() => togglePublish(post.id!, post.is_published)}
                      className="p-2 text-[#A8A6A0] hover:text-[#4A4A6A] border border-[#E2E0DC] rounded-lg hover:bg-[#F8F7F4] transition-all">
                    ...
                    <button onClick={() => del(post.id!)}
                      className="p-2 text-red-400 hover:text-red-600 border border-red-100 rounded-lg hover:bg-red-50 transition-all">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
...
// Note: unused imports removed — FileText is kept as it may be useful for future blog post type indicators
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _keepForFuture = FileText;
